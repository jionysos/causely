...
with tab_home:
    ...
    for col, icon, title, desc in [
        (t1, "🏃", "데이터 분석가 없는 스타트업 대표",
         'KPI가 왜 변했는지 원인 힌트를 즉시 파악! </br>분석가 없어도 빠른 의사결정이 가능합니다.'),
        (t2, "📣", "마케터 · PM",
         "원인 분석을 위한 분석은 그만! </br> 페인·채널·상품 중 무엇이 지표를 움직였는지 자동으로 찾아줍니다."),
        (t3, "📋", "데이터 분석가",
         "매번 반복되는 KPI 리포트 자동화! </br>의사결정에 집중할 수 있는 시간을 돌려드립니다."),
    ]:
        with col:
            st.markdown(f"""
            <div class="feature-card">
