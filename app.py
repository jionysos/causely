...
with tab_home:
    ...
    for col, icon, title, desc in [
        (t1, "🏃", "데이터 분석가 없는 스타트업 대표",
         'KPI가 왜 변했는지 원인 힌트를 즉시 파악! </br>분석가 없어도 빠르고 정확한 의사결정이 가능합니다.'),
        (t2, "📣", "마케터 · PM",
         "원인 파악을 위한 작업은 그만! </br> 캠페인·채널·상품 중 무엇이 지표를 움직였는지 찾아줍니다."),
        (t3, "📋", "데이터 분석가",
         "매번 반복되는 KPI 리포트 자동화! </br>의미있는 분석에 집중할 수 있는 시간을 돌려드립니다."),
    ]:
        with col:
            st.markdown(f"""
            <div class="feature-card">
...
with tab_analysis:
    ...
    with col_demo:
        st.markdown('<p style="color: #b91c1c; font-size: 1.35rem; font-weight: 700; margin: 0; text-align: center;">데모는 2달(25.12~26.1월) 데이터로 제공됩니다.</p>', unsafe_allow_html=True)
    ...
    try:
        ...
        if st.button("Causely 리포트 생성"):
            with st.spinner("리포트 생성 중…"):
                ...
                try:
                    report = core.generate_iv_report(components, today=today, compare_date=compare_date)
